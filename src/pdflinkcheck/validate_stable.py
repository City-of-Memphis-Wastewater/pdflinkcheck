...
def run_validation(
    ...
    if not all_links and not toc:
        ...
        return {"summary-stats": {"valid": 0, "broken": 0}, "issues": []} # incomplete, needs useful keys


    pdf_dir = Path(pdf_path).parent

    issues = []
    internal_page_jump_valid_count = 0 # add more granulaity for types of valid links
    file_found_count = 0
    web_ping_valid_count = 0
    toc_jump_valid_count = 0
    broken_file_count = 0
    broken_page_count = 0
    ...
    for link in all_links:
        ...
        #status = "valid" # defunct, all overwrite expected anyways, not 'innocent unless proven guilty"
        status = None
        reason = None
            ...
            if dest_page_raw is not None:
                ...
                try:
                    ...
                    # 4. Case: Perfect Match
                        ...
                        status = "internal-page-jump-valid"
        ...
        if status == "internal-page-jump-valid":
            internal_page_jump_valid_count += 1
        elif status =="file-found-valid":
            file_found_count += 1
        elif status == "web-ping-fail":
            web_ping_fail_count += 1
        elif status == "web-ping-valid":
            web_ping_valid_count += 1
            ...
            broken_file_count += 1
            issues.append(link_with_val)
        elif status == "no-destination-page":
    ...
    for entry in toc:
        try:
            ...
            # 4. Valid Case
                ...
                status = "toc_jump_valid"
                toc_jump_valid_count += 1
    ...
    summary_stats = {
        ...
        "internal-page-jump-valid": internal_page_jump_valid_count,
        "toc-jump-valid": toc_jump_valid_count,
        "file-found-valid": file_found_count,
        "broken-page": broken_page_count,
        "broken-file": broken_file_count,
        "no-destination-page": no_destination_page_count,
        "web-ping-fail":web_ping_fail_count,
        "web-ping-valid":web_ping_valid_count,
...
def generate_validation_summary_txt_buffer(summary_stats, issues, pdf_path, check_external):
    ...
    log(f"Total items found: {summary_stats['total_found']}")
    log(f"✅ TOC Page Jump Valid: {summary_stats['toc-jump-valid']}")
    log(f"✅ Interal Page Jump Valid: {summary_stats['internal-page-jump-valid']}")
    log(f"✅ File Targets Found: {summary_stats['file-found-valid']}")
    log(f"✅ Web Addresses Valid: {summary_stats['web-ping-valid']}")
    ...
    log(f"❌ No Destination Resolved: {summary_stats['no-destination-page']}")
